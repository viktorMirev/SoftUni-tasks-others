    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            while (true)
            {
              ...
              if (validation.Match(line).Value.Length == line.Length)
                {
                    ...
                    if (Events.ContainsKey(command[0]))
                    {
                        if (command[1].Substring(1) == Events[command[0]].Name)
                        {
                            ...
                            Events[command[0]].Participants = Events[command[0]].Participants.Distinct().ToList();
                        }
                        else continue;
                    }
                    else
                    {
                        Events.Add(command[0], new Event(command[1].Substring(1), command.Where(x => x[0] == '@').Distinct().ToList()));
